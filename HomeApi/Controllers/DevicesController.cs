...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        /// Удаление устройства
        /// </summary>
        /// <param name="name">Имя удаляемого устройства</param>
        /// <returns></returns>
        [HttpPost]
        [Route("deleteDevice/{name}")]
        public async Task<IActionResult> DeleteDevice([FromRoute] string name)
        {
            var device = await _devices.GetDeviceByName(name);
            if (device == null)
                return StatusCode(400, $"Ошибка: Устройство {name} не найдено.");
            await _devices.DeleteDevice(device);
            return StatusCode(200, $"Устройство {name} удалено.");
        }
    }
}